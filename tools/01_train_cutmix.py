...
class Stage1CutMixTrainer:
    ...
    def initialize_system(self):
        ...
        self.logger.info('=== Initiating Stage 1 CutMix Training Setup ===')
    ...
    def setup_data_loaders(self):
        self.training_dataset, self.training_data_loader, self.training_sampler = build_cutmix_dataloader(
    ...
    def setup_model(self):
        self.model = build_network(model_cfg=self.cfg.MODEL, num_class=1, dataset=self.training_dataset)
    ...
    def run_training(self):
        self.logger.info(f'*** Starting Stage 1 CutMix Training: {self.cfg.EXP_GROUP_PATH}/{self.cfg.TAG}({self.args.extra_tag}) ***')
        train_model(
            self.model,
        ...
        self.logger.info(f'*** Stage 1 CutMix Training Completed: {self.cfg.EXP_GROUP_PATH}/{self.cfg.TAG}({self.args.extra_tag}) ***\n\n\n')

    def run_evaluation(self):
        self.logger.info(f'*** Starting Model Evaluation on Test Dataset: {self.cfg.EXP_GROUP_PATH}/{self.cfg.TAG}({self.args.extra_tag}) ***')
        ...
        self.test_dataset, self.test_data_loader, self.test_sampler = build_dataloader(
            dataset_cfg=self.cfg.DATA_CONFIG_TEST,
            class_names=self.cfg.DATA_CONFIG_TEST.CLASS_NAMES,
            batch_size=self.args.batch_size,
        ...
        self.logger.info(f'*** Model Evaluation Completed: {self.cfg.EXP_GROUP_PATH}/{self.cfg.TAG}({self.args.extra_tag}) ***')


if __name__ == '__main__':
    arguments, configuration = load_training_arguments()
    trainer = Stage1CutMixTrainer(arguments, configuration)
